...
def main(args):
    ...
    weight = torch.from_numpy(dataLoad["classWeights"])  # convert the numpy array to torch
    ...
    for split in splits:
        ...
        #task_model = unet.UnetGenerator(in_dim=3, out_dim=args.classes, num_filter=8)
        #task_model = deeplab.DeepLab(num_classes=args.classes)
        ...
        args.initial_budget = int(args.num_images*split)
        ...
        final_mIOU, overall_acc, task_model = solver.train(
            querry_dataloader, val_dataloader, task_model, single_model, unlabeled_dataloader
        )
